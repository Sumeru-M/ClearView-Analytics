...
<<<<<<< HEAD
        ...
        # beta = 1 for all assets if the Nifty benchmark is unavailable.
        betas = None
        try:
            idx_prices = load_price_data(["^NSEI"], period="2y")
            if idx_prices is not None and not idx_prices.empty:
                idx_ret = compute_daily_returns(idx_prices, min_assets=1)
                betas = compute_market_betas(returns[valid], idx_ret.iloc[:, 0])
                result["betas"] = {t: round(float(betas.get(t, 1.0)), 4) for t in valid}
                result["portfolio_beta"] = round(
                    float(sum(weights.get(t, 0.0) * betas.get(t, 1.0) for t in valid)), 4
                )
        except Exception:
