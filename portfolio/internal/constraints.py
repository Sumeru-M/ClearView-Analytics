...
def apply_weight_bounds_numpy(
    ...
    *,
    ...
    """Clip and renormalise weights to satisfy box constraints (for HRP post-processing)."""
    ...
    for _ in range(max_iterations):
        ...
        if total <= 0:
            return np.ones(len(w)) / len(w)
        w /= total
        if np.all((w >= min_weight - 1e-9) & (w <= max_weight + 1e-9)):
            break
    return w


def apply_sector_caps_numpy(
    weights: np.ndarray,
    tickers: List[str],
    sector_map: Dict[str, str],
    sector_cap: float,
    *,
    ...
    """
    Iteratively scale down overweight sectors then renormalise (HRP / heuristic paths).
    """
    w = apply_weight_bounds_numpy(
        weights, min_weight=min_weight, max_weight=max_weight, max_iterations=max_iterations
    )
    sectors: Dict[str, List[int]] = {}
    for idx, ticker in enumerate(tickers):
        sectors.setdefault(sector_map.get(ticker, "Unknown"), []).append(idx)

    for _ in range(max_iterations):
        overflow = False
        for indices in sectors.values():
            sector_w = float(w[indices].sum())
            if sector_w > sector_cap + 1e-9:
                overflow = True
                scale = sector_cap / sector_w
                w[indices] *= scale
        w = apply_weight_bounds_numpy(
            w, min_weight=min_weight, max_weight=max_weight, max_iterations=20
        )
        if not overflow:
            break
    return w
