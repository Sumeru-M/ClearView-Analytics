...
def optimize_black_litterman(
    ...
    The Black-Litterman model fixes the two biggest weaknesses of plain
    mean-variance optimisation: (1) extreme, error-maximising weights driven
    by noisy sample-mean estimates, and (2) the need to specify a full vector
    of expected returns. It does this in two stages:

    Stage 1 — Reverse optimisation (the "prior")
        Start from a neutral benchmark portfolio ``market_weights`` and infer
        the expected returns that would make that portfolio optimal:
            Pi = delta * Sigma * w_market
        ``Pi`` is the *equilibrium implied return* vector. ``delta`` is the
        market risk-aversion coefficient (derived from the benchmark Sharpe
        ratio when not supplied).

    Stage 2 — Bayesian update with views (the "posterior")
        Investor/model views are expressed as ``P @ E[R] = Q`` with uncertainty
        ``Omega``. The posterior expected returns blend the equilibrium prior
        with the views:

            mu_BL = [ (tau*Sigma)^-1 + P' Omega^-1 P ]^-1
                    [ (tau*Sigma)^-1 Pi + P' Omega^-1 Q ]

        and the posterior covariance is ``Sigma_BL = Sigma + M`` where
        ``M = [ (tau*Sigma)^-1 + P' Omega^-1 P ]^-1``.

    The posterior ``(mu_BL, Sigma_BL)`` is then fed into a constrained
    mean-variance optimiser to produce the final weights. Because the views
    are shrunk toward equilibrium, the resulting allocation is far more stable
    and diversified than raw Markowitz.

    Parameters
    ----------
    mu : np.ndarray (N,)
        Annualised expected returns from the data. When no explicit views are
        supplied these are used as absolute views (``P = I``, ``Q = mu``),
        i.e. "I believe each asset earns its historical mean" — blended with
        the equilibrium prior according to ``view_confidence``.
    Sigma : np.ndarray (N, N)
        Annualised covariance matrix (e.g. Ledoit-Wolf shrunk).
    tickers : List[str]
    constraint_builder : ConstraintBuilder
        Long-only / max-weight / sector-cap constraints for the final MVO step.
    market_weights : np.ndarray (N,), optional
        Benchmark/prior weights for reverse optimisation. Defaults to equal
        weight (1/N) when not provided — a neutral, market-cap-agnostic prior.
    risk_aversion : float, optional
        delta. If None, derived as ``(w_mkt . mu - rf) / (w_mkt . Sigma . w_mkt)``
        and floored at a sensible positive value.
    tau : float, default 0.05
        Scalar on the prior covariance of the equilibrium estimate. Smaller
        ``tau`` => more confidence in the equilibrium prior.
    view_confidence : float, default 1.0
        Scales the view uncertainty ``Omega`` (Omega_ii = tau * p_i Sigma p_i'
        / view_confidence). Higher => views weighted more heavily vs. the prior.
    P, Q, Omega : np.ndarray, optional
        Explicit views. ``P`` is (K, N), ``Q`` is (K,), ``Omega`` is (K, K).
        If omitted, absolute views from ``mu`` are used (He & Litterman 2002
        proportional ``Omega``).
    returns_history : np.ndarray (T, N), optional
        Historical simple returns for the CVaR figure in the result.
    lam : float, default 1.0
        Risk-aversion in the final mean-variance step.
    rf : float, default 0.07
        Risk-free rate for Sharpe reporting.

    Returns
    -------
    AllocationResult
        optimization_type = "Black-Litterman"
        diagnostics carries delta, tau, implied (equilibrium) returns and the
        posterior Black-Litterman returns for transparency.
    """
    N = len(tickers)
    Sigma = np.asarray(Sigma, dtype=float)
    mu = np.asarray(mu, dtype=float).reshape(N)

    # ── Prior: market/benchmark weights ──────────────────────────────────────
    if market_weights is None:
        w_mkt = np.ones(N) / N
    else:
        w_mkt = np.asarray(market_weights, dtype=float).reshape(N)
        total = w_mkt.sum()
        w_mkt = w_mkt / total if total > 0 else np.ones(N) / N

    # ── Risk-aversion delta (market price of risk) ───────────────────────────
    if risk_aversion is None:
        mkt_var = float(w_mkt @ Sigma @ w_mkt)
        mkt_excess = float(w_mkt @ mu) - rf
        delta = mkt_excess / mkt_var if mkt_var > 1e-12 else 2.5
        # Clamp to a plausible band; literature commonly uses ~2-4.
        delta = float(np.clip(delta, 1.0, 10.0))
    else:
        delta = float(risk_aversion)

    # ── Stage 1: equilibrium implied returns  Pi = delta * Sigma * w_mkt ──────
    Pi = delta * (Sigma @ w_mkt)

    # ── Views: default to absolute views from the sample means ────────────────
    ...
    # He & Litterman (2002): Omega proportional to the view variance.
    if Omega is None:
        view_var = np.diag(P @ tau_Sigma @ P.T)
        conf = max(view_confidence, 1e-6)
        Omega = np.diag(np.maximum(view_var, 1e-10) / conf)
    else:
        Omega = np.atleast_2d(np.asarray(Omega, dtype=float))

    # ── Stage 2: Bayesian posterior (mu_BL, Sigma_BL) ─────────────────────────
    # Use pseudo-inverse throughout for numerical robustness on near-singular
    # shrunk covariances and small/correlated universes.
    ...
    # Symmetrise to wash out floating-point asymmetry before the QP.
    Sigma_bl = 0.5 * (Sigma_bl + Sigma_bl.T)

    # ── Final constrained mean-variance step on the posterior ─────────────────
    status = "optimal"
    if CVXPY_AVAILABLE:
        w = cp.Variable(N, name="weights")
        objective = cp.Maximize(mu_bl @ w - lam * cp.quad_form(w, cp.psd_wrap(Sigma_bl)))
        constraints = constraint_builder.build(w, Sigma=Sigma_bl, mu=mu_bl)
        prob = cp.Problem(objective, constraints)
        prob.solve(solver=cp.OSQP, verbose=False, eps_abs=1e-8, eps_rel=1e-8)
        status = prob.status or "failed"
        if w.value is None:
            w_val = w_mkt.copy()
            status = "failed_fallback_market_weights"
        else:
            w_val = np.maximum(w.value, 0)
            w_val = w_val / w_val.sum() if w_val.sum() > 0 else w_mkt.copy()
    else:
        # Closed-form unconstrained BL weights, then clip to long-only/bounds.
        w_val = np.linalg.pinv(delta * Sigma_bl) @ mu_bl
        w_val = np.maximum(w_val, 0)
        w_val = w_val / w_val.sum() if w_val.sum() > 0 else w_mkt.copy()
        status = "optimal_closed_form"

    res = _make_result(
        w_val, tickers, mu_bl, Sigma_bl, returns_history,
        "Black-Litterman", status, rf,
    )
    res.diagnostics = {
        "risk_aversion_delta": round(float(delta), 4),
        "tau": float(tau),
        "view_confidence": float(view_confidence),
        "n_views": int(K),
        "implied_equilibrium_returns": {
            t: round(float(v), 6) for t, v in zip(tickers, Pi)
        },
        "posterior_returns": {
            t: round(float(v), 6) for t, v in zip(tickers, mu_bl)
        },
        "market_weights": {
            t: round(float(v), 6) for t, v in zip(tickers, w_mkt)
        },
    }
    return res
