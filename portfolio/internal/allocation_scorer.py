...
def _score_diversification(weights: pd.Series, covariance_matrix: pd.DataFrame) -> float:
    ...
    DR_max=DR of 1/vol-weighted port  → 100 points

    The ceiling is the empirical maximum DR achievable from this specific
    covariance matrix (inverse-vol weighting), NOT the theoretical sqrt(N)
    which assumes uncorrelated assets and is unreachable for correlated
    equity portfolios. Using sqrt(N) would permanently understate HRP and
    any other diversifying optimizer when assets are correlated.
    """
    dr = compute_diversification_ratio(weights, covariance_matrix)

    # Empirical ceiling: inverse-vol weighted portfolio.
    # This is the practical maximum DR for a given covariance structure.
    # 1/vol weighting maximises the weighted-average vol in the DR numerator
    # while keeping portfolio vol low — it is the natural upper bound.
    std_devs = np.sqrt(np.diag(covariance_matrix.values))
    std_devs = np.maximum(std_devs, 1e-8)          # guard zero-vol edge case
    inv_vol = 1.0 / std_devs
    w_inv_vol = pd.Series(inv_vol / inv_vol.sum(), index=covariance_matrix.index)
    dr_max = compute_diversification_ratio(w_inv_vol, covariance_matrix)
